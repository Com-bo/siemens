...
} from '@/app/request/apiSpecialDivision';
const pageName = 'SpecialDivision';
...
export const Index = (props: any) => {
  ...
  const exportExcelAction = () => {
    ...
    SpecialDivisionExportData(params).then((res: any) => {
      if (res.response.status == 200) {
        let elink = document.createElement('a');
        // 设置下载文件名
        elink.download = 'SpecialDivision List.xlsx';
  ...
  const importExcel = (file) => {
    ...
    fd.append('file', file);
    SpecialDivisionImportData(fd).then((res) => {
      if (res.isSuccess) {
        res.data && parseExcel(res.data, 'SpecialDivision import feedback');
  ...
  const saveFormData = () => {
    formData
      ...
      .then((values) => {
        ...
        SpecialDivisionEditDataSave(params).then((res) => {
          if (res.isSuccess) {
            message.success(res.msg);
